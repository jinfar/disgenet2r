...
#' related to molecular functions where the gene product is involved and creates
...
gene2molecularfunction <- function( gene, verbose = FALSE, warnings = TRUE ) {
  ...
  message("Performing the query. This is a federated query with Wikidata and may take some time, please wait...")
    ...
    # oql is the query that let us look for GO MF in Wikidata
    query <- "PREFIX wd: <http://www.wikidata.org/entity/>
      PREFIX wdt: <http://www.wikidata.org/prop/direct/>

      SELECT DISTINCT str(?ncbigene_id) as ?ncbi_geneid ?gene_iri str(?gene_product_label) as ?gene_product_label str(?go_id) as ?go_id str(?go_label) as ?go_label ?molecular_function_iri WHERE {
      ...
      SERVICE <https://query.wikidata.org/sparql> {
        ...
        ?gene_product wdt:P680 ?molecular_function_iri . #add information about the GO cell component
        ?molecular_function_iri rdfs:label ?go_label ;
                        wdt:P686 ?go_id .
