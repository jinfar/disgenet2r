...
#'
#' Given a disease or a list of diseases, retrieves the GO CC of the gene products
...
#' @examples
#' disease2cc <- disease2cellcomponent( disease = "C1859062" )
#' @export disease2cellcomponent


disease2cellcomponent <- function( disease, vocabulary = "umls", database = "CURATED", score = c(0,1), verbose = FALSE, warnings = TRUE ) {
  ...
  for (gene in genes ){
    ...
    g2cc <- gene2cellcomponent(gene = gene )
    if(!is.null(g2cc)){
      results <- rbind(results, g2cc@qresult)
    } else{
